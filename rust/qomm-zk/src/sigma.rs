...
pub fn prove_opening<R: RngCore + CryptoRng>(
    key: &Pedersen, transcript: &mut Transcript, commitment: &RistrettoPoint,
    value: &Scalar, blinding: &Scalar, rng: &mut R,
) -> OpeningProof {
    ...
    OpeningProof { t, z_value: k_value + c * value, z_blinding: k_blinding + c * blinding }
}
...
pub fn opening_terms(
    key: &Pedersen, transcript: &mut Transcript, commitment: &RistrettoPoint,
    proof: &OpeningProof, weight: &Scalar,
    ...
    let c = transcript.challenge_scalar(b"c");
    (
        vec![weight * proof.z_value, weight * proof.z_blinding, -weight, -(weight * c)],
...
pub fn prove_same_value<R: RngCore + CryptoRng>(
    key: &Pedersen, transcript: &mut Transcript,
    first_generator: &RistrettoPoint, second_generator: &RistrettoPoint,
    first_commitment: &RistrettoPoint, second_commitment: &RistrettoPoint,
    value: &Scalar, first_blinding: &Scalar, second_blinding: &Scalar, rng: &mut R,
) -> CrossGeneratorProof {
    let k_value = Scalar::random(rng);
    ...
    let c = cross_challenge(transcript, first_generator, second_generator,
...
fn cross_challenge(
    transcript: &mut Transcript, g1: &RistrettoPoint, g2: &RistrettoPoint,
    c1: &RistrettoPoint, c2: &RistrettoPoint, t1: &RistrettoPoint, t2: &RistrettoPoint,
) -> Scalar {
...
pub fn same_value_terms(
    ...
    first_generator: &RistrettoPoint, second_generator: &RistrettoPoint,
    first_commitment: &RistrettoPoint, second_commitment: &RistrettoPoint,
    proof: &CrossGeneratorProof, weight: &Scalar,
) -> (Vec<Scalar>, Vec<RistrettoPoint>) {
    let c = cross_challenge(transcript, first_generator, second_generator,
    ...
    (
        vec![
            weight * proof.z_value, weight * proof.z_first, -weight, -(weight * c),
            w2 * proof.z_value, w2 * proof.z_second, -w2, -(w2 * c),
        ],
        vec![
            *first_generator, key.h, proof.t_first, *first_commitment,
            *second_generator, key.h, proof.t_second, *second_commitment,
        ],
    )
}
...
pub fn verify_same_value(
    ...
    let (scalars, points) = same_value_terms(
        key, transcript, first_generator, second_generator,
        first_commitment, second_commitment, proof, &Scalar::ONE);
    RistrettoPoint::vartime_multiscalar_mul(&scalars, &points) == RistrettoPoint::identity()
}
...
pub fn prove_product<R: RngCore + CryptoRng>(
    key: &Pedersen, transcript: &mut Transcript, c_a: &RistrettoPoint,
    a: &Scalar, r_a: &Scalar, b: &Scalar, r_b: &Scalar, r_c: &Scalar, rng: &mut R,
) -> ProductProof {
    ...
    ProductProof {
        t_factor, t_product,
        z_b: k_b + c * b, z_rb: k_rb + c * r_b, z_s: k_s + c * s,
    }
}

fn product_challenge(
    transcript: &mut Transcript, c_a: &RistrettoPoint, c_b: &RistrettoPoint,
    c_c: &RistrettoPoint, t_factor: &RistrettoPoint, t_product: &RistrettoPoint,
) -> Scalar {
...
pub fn product_terms(
    key: &Pedersen, transcript: &mut Transcript, c_a: &RistrettoPoint,
    c_b: &RistrettoPoint, c_c: &RistrettoPoint, proof: &ProductProof, weight: &Scalar,
) -> (Vec<Scalar>, Vec<RistrettoPoint>) {
    let c = product_challenge(transcript, c_a, c_b, c_c, &proof.t_factor, &proof.t_product);
    let w2 = weight * weight;
    (
        vec![
            weight * proof.z_b, weight * proof.z_rb, -weight, -(weight * c),
            w2 * proof.z_b, w2 * proof.z_s, -w2, -(w2 * c),
        ],
        vec![
            key.g, key.h, proof.t_factor, *c_b,
            *c_a, key.h, proof.t_product, *c_c,
        ],
    )
}
...
pub fn verify_product(
    key: &Pedersen, transcript: &mut Transcript, c_a: &RistrettoPoint,
    c_b: &RistrettoPoint, c_c: &RistrettoPoint, proof: &ProductProof,
) -> bool {
...
impl Batch {
    ...
    pub fn weight<R: RngCore + CryptoRng>(rng: &mut R) -> Scalar { Scalar::random(rng) }
...
pub fn prove_linear<R: RngCore + CryptoRng>(
    key: &Pedersen, transcript: &mut Transcript,
    commitments: &[RistrettoPoint], coefficients: &[Scalar], blindings: &[Scalar],
    constant: &Scalar, rng: &mut R,
) -> OpeningProof {
    let combined: Scalar = coefficients.iter().zip(blindings)
        .map(|(c, r)| c * r).sum();
...
pub fn verify_linear(
    key: &Pedersen, transcript: &mut Transcript,
    commitments: &[RistrettoPoint], coefficients: &[Scalar], constant: &Scalar,
    proof: &OpeningProof,
) -> bool {
    let residual = linear_residual(key, commitments, coefficients, constant);
...
fn linear_residual(
    key: &Pedersen, commitments: &[RistrettoPoint], coefficients: &[Scalar],
    constant: &Scalar,
) -> RistrettoPoint {
...
pub fn prove_bit<R: RngCore + CryptoRng>(
    key: &Pedersen, transcript: &mut Transcript, commitment: &RistrettoPoint,
    bit: bool, blinding: &Scalar, rng: &mut R,
) -> BitProof {
    ...
    let (t0, t1) = if bit { (t_fake, t_real) } else { (t_real, t_fake) };
    let total = bit_challenge(transcript, commitment, &t0, &t1);
    let c_real = total - c_fake;
    let z_real = k + c_real * blinding;

    if bit {
        BitProof { t0, t1, c0: c_fake, z0: z_fake, z1: z_real }
    } else {
        BitProof { t0, t1, c0: c_real, z0: z_real, z1: z_fake }
    }
}
...
fn bit_challenge(
    transcript: &mut Transcript, commitment: &RistrettoPoint,
    t0: &RistrettoPoint, t1: &RistrettoPoint,
) -> Scalar {
