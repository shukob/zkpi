...
fn main() {
    ...
    let mut venue = Venue::new(Pedersen::new(b"qomm:defmi:v1"), &Bounds::default(),
                               package);
    venue.domain = qomm_zkpi::DEFAULT_DOMAIN.to_vec();
    let now = instruction.deadline - 1;

    let encoding = time_us(200, || { encode(&instruction); });
    let decoding = time_us(200, || { decode(&bytes).unwrap(); });
    let verifying = time_us(50, || { venue.verify(&instruction, now).unwrap(); });
    ...
    println!("  off the wire    {:.1} us  (decode and verify)", whole.median);
    ...
    println!("  a whole verification as a precompile  {:>10.0} gas", as_precompile);
    println!("  one scalar mult in EVM bytecode       {:>10.0} gas  (measured)",
             IN_EVM_SCALAR_MUL_GAS);
    println!("  ratio                                 {:>10.1}x",
...
build-time configuration.\"\n}}\n",
            ...
            shell("rustc", &["--version"]), bytes.len(), encoding.json(),
            decoding.json(), verifying.json(), whole.json(), as_precompile,
            IN_EVM_SCALAR_MUL_GAS, IN_EVM_SCALAR_MUL_GAS / as_precompile,
            ECRECOVER_GAS, ECRECOVER_US);
